...
- `docker-compose.yml`, `Dockerfile`
  - Define the Docker-only runtime assumption. Profiles replace the old overrides; keep documented ports aligned with these manifests.
...
- **ingress-yahoo** – local Yahoo ingress on http://localhost:8080
- **gapfill-cron**, **tickets-cron** – background jobs
...
API: make up (starts API + supporting stack)

Dashboard options

- Compose dashboard: served at http://localhost:5180
- Dev profile (Vite hot reload): `make up-dev` → http://localhost:5179

Verify health

curl -fsS http://localhost:3000/health → {"ok":true}

Dashboard loads (5180 compose, 5179 dev)

Confirm tickets path

API writes to /data/tickets/YYYY-MM-DD/*.json (volume api-data); tickets-cron keeps /data/tickets.jsonl aggregated
...
> Repo scan appendix removed after compose profile refresh. Refer to README.md for stack inventory.
...
2. Legacy compose override artefacts (`docker-compose.override.dashboard.yml`, etc.) are intentionally retired; ensure downstream docs reference profiles instead.
...
- [ ] `docker-compose.yml` (with prod/jobs profiles) deployed to server.
