...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE t.tgname = 'orr_config_set_updated_at'
      AND n.nspname = 'public'
      AND c.relname = 'orr_config'
  ) THEN
    EXECUTE $sql$
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN pg_class c ON c.oid = t.tgrelid
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE t.tgname = 'osb_config_set_updated_at'
      AND n.nspname = 'public'
      AND c.relname = 'osb_config'
  ) THEN
    EXECUTE $sql$
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN pg_class c ON c.oid = t.tgrelid
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE t.tgname = 'vwap_ft_config_set_updated_at'
      AND n.nspname = 'public'
      AND c.relname = 'vwap_ft_config'
  ) THEN
    EXECUTE $sql$
