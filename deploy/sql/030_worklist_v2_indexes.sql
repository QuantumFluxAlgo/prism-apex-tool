...
$$
DECLARE
  has_engine_opened BOOLEAN := (
    SELECT EXISTS (
      SELECT 1
      FROM information_schema.columns
      WHERE table_schema = 'public'
        AND table_name = 'engine_tickets'
        AND column_name = 'opened_at_utc'
    )
  );
BEGIN
  ...
  IF has_engine_opened THEN
    ...
    END IF;
  ELSE
    RAISE NOTICE 'Skipping engine_tickets worklist index – opened_at_utc column missing';
  END IF;
END;
$$;
