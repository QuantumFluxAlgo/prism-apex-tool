...
**Goal**  
Enforce a consistent A2 visual language across all pages, with a single way to display prices and risk.

### Stories

- [ ] **V2.6.1 – Design Tokens & Component Audit**  
  - Codify A2 tokens (colours, typography, spacing, radii, shadows).  
  - Audit existing components; refactor obvious outliers.  
  - Outcome: a documented, enforced A2 design system.

- [ ] **V2.6.2 – Table & Card Standardisation**  
  - Ensure all tables share the same base patterns and class names.  
  - Align card headers, body spacing, and typography across pages.  
  - Outcome: all pages clearly belong to the same product.
...
- [ ] **V2.6.4 – UI-Level Canonical Integrity Checks**  
  - Add guards to fail fast if API responses violate core assumptions (e.g. invalid price ordering).  
  - Outcome: UI won’t silently render nonsensical ticket data.
