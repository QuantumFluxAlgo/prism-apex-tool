...
> This document describes the structure of the `prism-apex-tool` monorepo as of the current V2 snapshot.
> It is intended to be the **canonical map** for architecture, data flows, and key modules used by the Prism Apex V2 dashboard and engine.
...
**Key stores (`apps/api/src/store`) – all _CANONICAL_:**
...
- A2-variant operator dashboard for:
  ...
  - System Status
  - Alerts
- All V2 surfaces must use `ExecutionShell` and A2 UI primitives.
...
**Canonical V2 dashboard pages (`apps/dashboard/src/pages`) – _CANONICAL_:**
...
#### 2.2.3 UI Primitives (`apps/dashboard/src/ui`) – _CANONICAL_
...
These are the **preferred building blocks** for all V2 UI work.
...
**Key files (all _CANONICAL_ for typing):**
...
- `docs/ui/PRISM_APEX_UI_DESIGN_SYSTEM.md` – A2 visual language and UI foundations.
- `docs/ui/specs/worklist.md` – Worklist UI spec (V2).
- `docs/ui/specs/tickets.md` – Tickets UI spec.
- `docs/ui/specs/markets.md` – Markets/MarketData UI spec.
...
_All of these are **CANONICAL** routes unless explicitly prefixed as test-only._
...
_All of these are **CANONICAL** jobs unless explicitly marked as test-only._
...
These must **not** be treated as canonical data sources. They are for demos and UI-only exploration.
...
1. Any time a **page**, **route**, **job**, **store**, or **canonical DTO** is added/moved/deleted, this file must be updated in the same PR.
2. Any time a new **mock**, **fixture**, or **demo server** is added, it must be recorded here as `MOCK` or `TEST_FIXTURE`.
3. When EPICs or dashboard scope change, update:
   - `docs/PRISM_APEX_V2_DASHBOARD_PLAN.md`
   - This file (`docs/REPO_INDEX_V2.md`)
4. Future ChatGPT sessions must treat this file as the **map**, then confirm via recon (Codex Terminal) before changing behaviour.
