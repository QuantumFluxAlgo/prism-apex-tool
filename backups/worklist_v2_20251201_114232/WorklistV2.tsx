...
function deriveScore(ticket: EnrichedTicket): number {
  ...
  if (metrics) {
    ...
    const direction = ticket.side === 'LONG' ? 'UP' : 'DOWN';
...
export default function WorklistV2Page() {
  ...
  React.useEffect(() => {
    ...
    fetchWorklistCanonicalTickets()
      ...
      .catch(() => {
        // no mock fallback – if this fails, the table will show the empty state
      });
    return () => {
      cancelled = true;
    };
  ...
  }, [apiTickets, sessionMetricsMap, usingApiTickets]);
      ...
      },
      {
        ...
        render: (ticket) =>
          renderRowCell(
            ...
            selectedId,
            handleSelect,
            <span className="font-mono text-white">{fmtR(ticket.rrMultiple)}</span>,
          ),
      },
      {
        key: 'context',
    ...
    <ExecutionShell activeTab="worklist">
      <section className="space-y-4">
        <header className="rounded-3xl border border-slate-800 bg-slate-900/70 p-5 text-slate-100 shadow-xl">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-semibold tracking-tight text-white">Worklist</h2>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <CardHeader>
              <div className="flex items-center justify-between text-slate-200">
                <div>
                  ...
                  <h3 className="text-xl font-semibold">{filteredTickets.length} Ready</h3>
                </div>
                <Badge tone="gray">
                  Maximum age {filters.maxAge === 'ANY' ? MAX_AGE_MINUTES : filters.maxAge}
                  m
                </Badge>
              </div>
            </CardHeader>
          ...
          <Card className="rounded-3xl border border-slate-800 bg-slate-900/60 shadow-xl">
            <CardHeader>
              <h3 className="text-xl font-semibold text-white">Signal Details</h3>
            </CardHeader>
                ...
                <div className="space-y-4">
                  ...
                  <DetailsSection title="Market Context">
                    <div className="flex flex-wrap gap-2">
                      ...
                      {getContextTags(selectedTicket).length === 0 ? (
                        <span className="text-xs text-slate-500">No context tags.</span>
                      ) : null}
                    </div>
                    {(() => {
                      ...
                      if (metrics) {
                        return (
                          <div className="mt-3 space-y-2 border-t border-slate-800 pt-3">
                            ...
                            <InfoRow
                              label="News"
                              value={metrics.hasMajorNewsToday ? metrics.newsLabel ?? 'Major event' : 'None'}
                            />
                          </div>
                        );
                      }
                  ...
                  <DetailsSection title="Timing">
                    <div className="space-y-2">
                      ...
                      <InfoRow
                        label="Completed"
                        value={selectedTicket.completedAtUtc ? fmtUtc(selectedTicket.completedAtUtc) : '—'}
                      />
                    </div>
                  </DetailsSection>
