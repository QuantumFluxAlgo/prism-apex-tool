...
const sampleSnapshot = {
  ...
  fills: [{ accountId: 'A1', contract: 'ESZ4', side: 'SELL', qty: 1, price: 110, ts: new Date().toISOString() }],
...
describe('telemetry API', () => {
  it('exposes telemetry via routes and provider', async () => {
    ...
    const fills = await app.inject({ method: 'GET', url: `/telemetry/fills?accountId=A1&date=${sampleSnapshot.dailyPnL[0].date}` });
    expect(fills.json()).toHaveLength(1);
    const report = await app.inject({ method: 'GET', url: `/report/consistency?accountId=A1&window=1` });
