  ...
  return (
    <div className="rounded-2xl shadow p-4 bg-white border">
      ...
      <div className="text-sm">
        <div>
          Net Liq: <span className="font-mono">{netLiq != null ? netLiq.toFixed(2) : '—'}</span>
        </div>
        <div>
          DD Line: <span className="font-mono">{ddLine != null ? ddLine.toFixed(2) : '—'}</span>
        </div>
        <div
          className={`mt-2 inline-block px-2 py-1 rounded ${ok ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
        >
          {dist != null
            ? ok
              ? `Headroom: ${dist.toFixed(2)}`
              : `BREACH RISK: ${(-dist).toFixed(2)}`
            : 'No data'}
        </div>
      </div>
    </div>
