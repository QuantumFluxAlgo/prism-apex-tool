...
function isSessionMetricsDto(
  value: SessionMetricsEnrichment | undefined,
): value is SessionMetricsDto {
  if (!value) return false;
  if ((value as any).status === 'MISSING') return false;
  return typeof (value as any).sessionDate === 'string';
}
...
function formatSessionMetricsSummary(metrics: SessionMetricsDto | null): string | null {
  ...
  const parts: string[] = [];
  if (typeof (metrics as any).orWidthPoints === 'number') {
    parts.push(`OR ${formatPoints((metrics as any).orWidthPoints)}`);
  }
  if (typeof (metrics as any).sessionAtrPoints === 'number') {
    parts.push(`ATR ${formatPoints((metrics as any).sessionAtrPoints)}`);
  }
  if ((metrics as any).vwapSlope) parts.push(`VWAP ${(metrics as any).vwapSlope}`);
  if ((metrics as any).htfTrendBias) parts.push(`Trend ${(metrics as any).htfTrendBias}`);
  if ((metrics as any).hasMajorNewsToday) parts.push('News risk');
...
function deriveScore(ticket: EnrichedTicket): number {
  ...
  if (metrics) {
    const m: any = metrics;
    const ratio =
      typeof m.orWidthToAtrRatio === 'number'
        ? m.orWidthToAtrRatio
        : typeof m.orWidthPoints === 'number' &&
          typeof m.sessionAtrPoints === 'number' &&
          m.sessionAtrPoints !== 0
        ? m.orWidthPoints / m.sessionAtrPoints
        : null;
    ...
    const slope = m.vwapSlope?.toUpperCase?.();
    const bias = m.htfTrendBias?.toUpperCase?.();
    if (slope?.includes(direction) || bias?.includes(direction)) score += 6;
    if (slope?.includes(opposing) || bias?.includes(opposing)) score -= 6;
    if (m.hasMajorNewsToday) score -= 5;
    if (m.sessionQualityFlag === 'SKIP' || m.sessionSkipReason) score -= 5;
    if (m.status === 'ERROR') score -= 3;
  } else if ((ticket.sessionMetrics as any)?.status === 'MISSING') {
    score -= 4;
  }
...
function deriveStrength(ticket: EnrichedTicket): StrengthTrend {
  ...
  if (metrics) {
    ...
    if (m.hasMajorNewsToday) headwinds += 1;
    if (alignment >= 2 && headwinds === 0) return 'strong';
    if (headwinds >= 2 || m.sessionQualityFlag === 'SKIP') return 'weak';
    return 'neutral';
  }
...
export default function WorklistV2Page() {
  ...
  const allTickets = React.useMemo<EnrichedTicket[]>(() => {
    ...
    return source.map((ticket) => {
      const mapKey = makeSessionMetricsKey(ticket.symbol, ticket.sessionDateUtc);
  ...
  React.useEffect(() => {
    ...
    const pendingEntries = Array.from(unique.entries()).filter(
    ...
    const requests = pendingEntries.map(([, query]) => ({
      symbol: query.symbol,
      sessionDate: query.sessionDate,
    }));

    fetchSessionMetricsBatch(requests)
      .then((batchResult) => {
        if (cancelled) return;
        setSessionMetricsMap((prev) => {
          const next = { ...prev };
          pendingEntries.forEach(([key]) => {
            const metrics = batchResult[key] ?? null;
            next[key] = metrics ?? SESSION_METRICS_PLACEHOLDER;
          });
          return next;
        });
      })
      .catch(() => {
        ...
        setSessionMetricsMap((prev) => {
          ...
          pendingEntries.forEach(([key]) => {
            if (!(key in next)) {
              next[key] = SESSION_METRICS_PLACEHOLDER;
            }
          });
          return next;
        });
          ...
          renderRowCell(
            ...
            <div className="flex flex-col gap-1">
              ...
              {(() => {
                ...
                if ((ticket.sessionMetrics as any)?.status === 'MISSING') {
                  return (
  ...
  return (
    <section className="worklist-v2-root space-y-5">
      {/* A3 header – no "Execution" label */}
      <header className="worklist-v2-header">
      ...
      <div className="worklist-v2-filters">
        <FiltersBar
          selects={[
            {
              label: 'Symbol',
      ...
      <div className="worklist-v2-layout">
        <div className="panel worklist-v2-panel min-w-0 flex-1">
        ...
        <div className="panel details-panel worklist-v2-details w-full max-w-[380px] shrink-0 lg:max-w-full">
              ...
              <div className="space-y-4">
                ...
                <DetailsSection title="Market Context">
                  ...
                  {(() => {
                    ...
                    if (metrics) {
                      return (
                        <div className="mt-3 space-y-2 border-t border-slate-800 pt-3">
                          ...
                          <InfoRow
                            label="OR Width"
                            value={formatPoints(m.orWidthPoints)}
                          />
                          <InfoRow
                            label="Session ATR"
                            value={formatPoints(m.sessionAtrPoints)}
                          />
                          <InfoRow
                            label="VWAP Slope"
                            value={m.vwapSlope ?? '—'}
                    ...
                    if ((selectedTicket.sessionMetrics as any)?.status === 'MISSING') {
                      return (
